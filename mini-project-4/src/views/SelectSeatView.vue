<template>
    <div class="container">
        ...
        <table class="table table-bordered text-center">
            <tr>
                <td v-for="(item, index) in getKoltukList('Çiftli', 'Cam Kenarı')" :key="index">
                    <button
                        :class="{ 'koltuk': item.durum === 0, 'koltuk koltuk-dolu': item.durum === 1, 'koltuk koltuk-rez': item.durum === 2 }"
                        :disabled="item.durum === 1" @click="koltuk_sec(item.id)">
                        {{ item.numara }}
                    </button>
                </td>
            </tr>
            <tr>
                <td v-for="(item, index) in getKoltukList('Çiftli', 'Koridor')" :key="index">
                    <button
                        :class="{ 'koltuk': item.durum === 0, 'koltuk koltuk-dolu': item.durum === 1, 'koltuk koltuk-rez': item.durum === 2 }"
                        :disabled="item.durum === 1" @click="koltuk_sec(item.id)">
                        {{ item.numara }}
            ...
            <tr>
                <td v-for="(item, index) in getKoltukList('Tekli', 'Cam Kenarı')" :key="index">
                    <button
                        :class="{ 'koltuk': item.durum === 0, 'koltuk koltuk-dolu': item.durum === 1, 'koltuk koltuk-rez': item.durum === 2 }"
                        :disabled="item.durum === 1" @click="koltuk_sec(item.id)">
                        {{ item.numara }}
        ...
        <button class="btn btn-success float-right" @click="yolcu_bilgileri_gir" v-if="secilen_koltuklar.length > 0">
...
export default {
    ...
    methods: {
        ...
        yolcu_bilgileri_gir() {
            ...
            this.$router.push({ name: 'passengerinfo' });
        }
    }
}
</script>
